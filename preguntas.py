...
def pregunta_11():
    ...
    39   39    a,d,f
    """
    df = tbl1.sort_values(by="_c4")
    df = df.groupby("_c0")["_c4"].apply(lambda x: ",".join(x)).reset_index()
    return df
...
def pregunta_12():
    ...
    """
    df = tbl2.sort_values(by='_c5a')
    df = df.groupby('_c0').apply(lambda x: ",".join(x['_c5a']+':'+x['_c5b'].astype(str)))
    df = df.reset_index()
    df.columns = ['_c0', '_c5']
    return df
